...
class Title_frame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # --->title widget
        self.title_page_title = ctk.CTkLabel(
            self, text="Kays Cafe", font=("Roboto", 70)
        )
        self.title_page_title.pack(pady=(30, 30))

        # --->logo widget
        self.title_page_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo2.png"), size=(350, 350)
        )
        self.title_page_logo_widget = ctk.CTkLabel(
            self, image=self.title_page_logo, fg_color="#161219", text=""
        )
        self.title_page_logo_widget.pack()

        # --->slogan widget
        self.title_page_slogan = ctk.CTkLabel(
            self, text="The cafe with attitude", font=("Roboto", 30)
        )
        self.title_page_slogan.pack(pady=(80, 0))

        # --->order button widget
        self.title_page_button = ctk.CTkButton(
            self,
            text="Order online, if you insist...",
            font=("Roboto", 70),
            cursor="hand2",
            border_spacing=20,
            command=lambda: controller.show_frame(Menu_navigation_frame),
        )
...
class Menu_navigation_frame(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        # --->top row frame
        self.menu_navigation_top_frame = ctk.CTkFrame(
            self,
        )

        # --->logo
        self.menu_navigation_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.menu_navigation_logo_widget = ctk.CTkLabel(
            self.menu_navigation_top_frame, image=self.menu_navigation_logo, text=""
        )

        # --->title
        self.menu_navigation_title = ctk.CTkLabel(
            self.menu_navigation_top_frame,
            textvariable=self.menu_navigation_title_var,
            font=("TkHeadingFont", 30),
        )

        # --->frame for basket button and basket total
        self.menu_navigation_basket_frame = ctk.CTkFrame(
            self.menu_navigation_top_frame,
        )

        # --->basket navigation
        self.menu_navigation_basket_button = ctk.CTkButton(
            self.menu_navigation_basket_frame,
            textvariable=self.menu_navigation_basket_button_var,
        ...
        # --->burger menu widget
        self.burgers_button = ctk.CTkButton(
            self.menu_navigation_buttons_frame,
            textvariable=self.burgers_button_var,
            command=lambda: controller.show_frame(Burger_menu),
        )

        # --->not burgers menu widget
        self.not_burgers_button = ctk.CTkButton(
            self.menu_navigation_buttons_frame,
            textvariable=self.not_burgers_button_var,
            command=lambda: controller.show_frame(Not_burger_menu),
        )

        # --->desserts menu widget
        self.desserts_button = ctk.CTkButton(
            self.menu_navigation_buttons_frame,
            textvariable=self.desserts_button_var,
            command=lambda: controller.show_frame(Dessert_menu),
        )

        # --->drinks menu widget
        self.drinks_button = ctk.CTkButton(
            self.menu_navigation_buttons_frame,
            textvariable=self.drinks_button_var,
            command=lambda: controller.show_frame(Drinks_menu),
        )

        # --->milkshakes menu widget
        self.milkshakes_button = ctk.CTkButton(
            self.menu_navigation_buttons_frame,
            textvariable=self.milkshakes_button_var,
            command=lambda: controller.show_frame(Milkshake_menu),
        )

        # ---text/photo frame
        self.text_photo_frame = ctk.CTkFrame(self, border_width=0)
        self.welcome_label = ctk.CTkLabel(
            self.text_photo_frame, textvariable=self.welcome_var
        )
        ...
        # --->facebook widget
        facebook_logo = ctk.CTkImage(
            Image.open(r"Project\images\facebook_logo.gif"), size=(30, 30)
        )
        facebook_logo_widget = ctk.CTkButton(
            self.menu_navigation_bottom_frame,
            text="",
            image=facebook_logo,
            height=30,
            width=30,
        ...
        # --->instagram widget
        instagram_logo = ctk.CTkImage(
            Image.open(r"Project\images\instagram_logo.png"), size=(30, 30)
        )
        instagram_logo_widget = ctk.CTkButton(
            self.menu_navigation_bottom_frame,
            text="",
        ...
        # --->menu buttons
        self.burgers_button.configure(
            command=lambda: controller.show_frame(Burger_menu)
        )
        self.not_burgers_button.configure(
            command=lambda: controller.show_frame(Not_burger_menu)
        )
        self.desserts_button.configure(
            command=lambda: controller.show_frame(Dessert_menu)
        )
        self.drinks_button.configure(command=lambda: controller.show_frame(Drinks_menu))
        self.milkshakes_button.configure(
            command=lambda: controller.show_frame(Milkshake_menu)
        )
        ...
        # --->top row frame
        self.menu_navigation_top_frame.grid(
            row=0, column=0, columnspan=7, ipady=20, sticky="nsew"
        )
        ...
        # --->frame for basket button and basket total
        self.menu_navigation_basket_frame.grid(
            row=0, column=6, padx=(0, 5), pady=6, sticky="nsew"
        )
        ...
        # --->menu navigation buttons frame
        self.menu_navigation_buttons_frame.grid(
            row=1, column=0, rowspan=5, columnspan=2, sticky="nsew"
        )

        # --->burger menu widget
        self.burgers_button.grid(
            column=0,
            row=0,
            columnspan=2,
            ipady=60,
        ...
        # --->not burgers menu widget
        self.not_burgers_button.grid(
            column=0, row=1, columnspan=2, ipady=60, padx=20, pady=10, sticky="nsew"
        )

        # --->desserts menu widget
        self.desserts_button.grid(
            column=0, row=2, columnspan=2, ipady=60, padx=20, pady=10, sticky="nsew"
        )

        # --->drinks menu widget
        self.drinks_button.grid(
            column=0, row=3, columnspan=2, ipady=60, padx=20, pady=10, sticky="nsew"
        )

        # --->milkshakes menu widget
        self.milkshakes_button.grid(
            column=0,
            row=4,
            columnspan=2,
            ipady=60,
        ...
        # ---text/photo frame
        self.text_photo_frame.grid(
            row=1, column=2, rowspan=5, columnspan=5, sticky="nsew"
        )
        ...
        # --->bottom row frame
        self.menu_navigation_bottom_frame.grid(
            row=6, column=0, columnspan=7, ipady=20, sticky="nsew"
        )
...
class Burger_menu(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.burger_var1 = tk.StringVar(
            self, f"{burger_option_1.product}\n\n £{burger_option_1.price}"
        )
        self.burger_var2 = tk.StringVar(
            self, f"{burger_option_2.product}\n\n £{burger_option_2.price}"
        )
        self.burger_var3 = tk.StringVar(
            self, f"{burger_option_3.product}\n\n £{burger_option_3.price}"
        )
        self.chick_burger_var1 = tk.StringVar(
            self, f"{chick_burger_option_1.product}\n\n £{chick_burger_option_1.price}"
        )
        self.chick_burger_var2 = tk.StringVar(
            self, f"{chick_burger_option_2.product}\n\n £{chick_burger_option_2.price}"
        )
        self.chick_burger_var3 = tk.StringVar(
            self, f"{chick_burger_option_3.product}\n\n £{chick_burger_option_3.price}"
        )
        self.vegetarian_var = tk.StringVar(
            self, f"{vegetarian_option_1.product}\n\n £{vegetarian_option_1.price}"
        )
        self.vegan_var = tk.StringVar(
            self, f"{vegan_option_1.product}\n\n £{vegan_option_1.price}"
        )
        ...
        self.chick_burger_description1 = tk.StringVar(
            self, chick_burger_option_1.description
        )
        self.chick_burger_description2 = tk.StringVar(
            self, chick_burger_option_2.description
        )
        self.chick_burger_description3 = tk.StringVar(
            self, chick_burger_option_3.description
        )
        ...
        self.chick_burger_basket1 = tk.StringVar(
            self, f"£{chick_burger_option_1.price}"
        )
        self.chick_burger_basket2 = tk.StringVar(
            self, f"£{chick_burger_option_2.price}"
        )
        self.chick_burger_basket3 = tk.StringVar(
            self, f"£{chick_burger_option_3.price}"
        )
        ...
        # --->logo
        self.burger_menu_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.burger_menu_logo_widget = ctk.CTkLabel(
            self.burger_menu_top_frame, image=self.burger_menu_logo, text=""
        )
        ...
        self.chick_burger_option1 = ctk.CTkButton(
            self, textvariable=self.chick_burger_var1
        )
        self.chick_burger_option2 = ctk.CTkButton(
            self, textvariable=self.chick_burger_var2
        )
        self.chick_burger_option3 = ctk.CTkButton(
            self, textvariable=self.chick_burger_var3
        )
        ...
        # --->option buttons
        self.burger_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                burger_option_1.product,
                self.burger_description1,
                self.burger_basket1,
                burger_option_1,
                Burger_menu,
            )
        )
        self.burger_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                burger_option_2.product,
                self.burger_description2,
                self.burger_basket2,
                burger_option_2,
                Burger_menu,
            )
        )
        self.burger_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                burger_option_3.product,
                self.burger_description3,
                self.burger_basket3,
                burger_option_3,
                Burger_menu,
            )
        )
        self.chick_burger_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                chick_burger_option_1.product,
                self.chick_burger_description1,
                self.chick_burger_basket1,
                chick_burger_option_1,
                Burger_menu,
            )
        )
        self.chick_burger_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                chick_burger_option_2.product,
                self.chick_burger_description2,
                self.chick_burger_basket2,
                chick_burger_option_2,
                Burger_menu,
            )
        )
        self.chick_burger_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                chick_burger_option_3.product,
                self.chick_burger_description3,
                self.chick_burger_basket3,
                chick_burger_option_3,
                Burger_menu,
            )
        )
        self.v_burger_option.configure(
            command=lambda: add_to_basket_window(
                self,
                vegetarian_option_1.product,
                self.v_burger_description,
                self.v_burger_basket,
                vegetarian_option_1,
                Burger_menu,
            )
        )
        self.ve_burger_option.configure(
            command=lambda: add_to_basket_window(
                self,
                vegan_option_1.product,
                self.ve_burger_description,
                self.ve_burger_basket,
                vegan_option_1,
                Burger_menu,
            )
        )
        ...
        # --->frame for basket button and basket total
        self.burger_menu_basket_frame.grid(
            row=0, column=2, padx=(0, 5), pady=6, sticky="nsew"
        )
...
class Not_burger_menu(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.not_burger_var1 = tk.StringVar(
            self, f"{not_burger_option_1.product} \n\n £{not_burger_option_1.price}"
        )
        self.not_burger_var2 = tk.StringVar(
            self, f"{not_burger_option_2.product} \n\n £{not_burger_option_2.price}"
        )
        self.not_burger_var3 = tk.StringVar(
            self, f"{not_burger_option_3.product} \n\n £{not_burger_option_3.price}"
        )
        self.not_burger_var4 = tk.StringVar(
            self, f"{not_burger_option_4.product} \n\n £{not_burger_option_4.price}"
        )
        self.not_burger_var5 = tk.StringVar(
            self, f"{not_burger_option_5.product} \n\n £{not_burger_option_5.price}"
        )
        self.not_burger_var6 = tk.StringVar(
            self, f"{not_burger_option_6.product} \n\n £{not_burger_option_6.price}"
        )
        self.not_burger_var7 = tk.StringVar(
            self, f"{not_burger_option_7.product} \n\n £{not_burger_option_7.price}"
        )
        self.not_burger_var8 = tk.StringVar(
            self, f"{not_burger_option_8.product} \n\n £{not_burger_option_8.price}"
        )
        self.not_burger_var9 = tk.StringVar(
            self, f"{not_burger_option_9.product} \n\n £{not_burger_option_9.price}"
        )

        # --->item description

        self.not_burger_description1 = tk.StringVar(
            self, not_burger_option_1.description
        )
        self.not_burger_description2 = tk.StringVar(
            self, not_burger_option_2.description
        )
        self.not_burger_description3 = tk.StringVar(
            self, not_burger_option_3.description
        )
        self.not_burger_description4 = tk.StringVar(
            self, not_burger_option_4.description
        )
        self.not_burger_description5 = tk.StringVar(
            self, not_burger_option_5.description
        )
        self.not_burger_description6 = tk.StringVar(
            self, not_burger_option_6.description
        )
        self.not_burger_description7 = tk.StringVar(
            self, not_burger_option_7.description
        )
        self.not_burger_description8 = tk.StringVar(
            self, not_burger_option_8.description
        )
        self.not_burger_description9 = tk.StringVar(
            self, not_burger_option_9.description
        )
        ...
        # --->logo
        self.not_burger_menu_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.not_burger_menu_logo_widget = ctk.CTkLabel(
            self.not_burger_menu_top_frame, image=self.not_burger_menu_logo, text=""
        )

        # --->title
        self.not_burger_menu_title = ctk.CTkLabel(
            self.not_burger_menu_top_frame,
            textvariable=self.not_burger_title_var,
            font=("TkHeadingFont", 30),
        )

        # --->frame for basket button and basket total
        self.not_burger_menu_basket_frame = ctk.CTkFrame(self.not_burger_menu_top_frame)
        self.not_burger_menu_basket_frame.grid(
            row=0, column=2, padx=(0, 5), pady=6, sticky="nsew"
        )

        # --->basket navigation
        self.not_burger_menu_basket_button = ctk.CTkButton(
            self.not_burger_menu_basket_frame,
            textvariable=self.not_burger_basket_button_var,
            font=("TkHeadingFont", 10),
            height=10,
            width=30,
            border_width=0,
        ...
        # --->back button
        self.not_burger_back = ctk.CTkButton(
            self, textvariable=self.not_burger_back_var
        )
        ...
        # --->item buttons
        self.not_burger_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_1.product,
                self.not_burger_description1,
                self.not_burger_basket1,
                not_burger_option_1,
                Not_burger_menu,
            )
        )
        self.not_burger_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_2.product,
                self.not_burger_description2,
                self.not_burger_basket2,
                not_burger_option_2,
                Not_burger_menu,
            )
        )
        self.not_burger_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_3.product,
                self.not_burger_description3,
                self.not_burger_basket3,
                not_burger_option_3,
                Not_burger_menu,
            )
        )
        self.not_burger_option4.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_4.product,
                self.not_burger_description4,
                self.not_burger_basket4,
                not_burger_option_4,
                Not_burger_menu,
            )
        )
        self.not_burger_option5.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_5.product,
                self.not_burger_description5,
                self.not_burger_basket5,
                not_burger_option_5,
                Not_burger_menu,
            )
        )
        self.not_burger_option6.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_6.product,
                self.not_burger_description6,
                self.not_burger_basket6,
                not_burger_option_6,
                Not_burger_menu,
            )
        )
        self.not_burger_option7.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_7.product,
                self.not_burger_description7,
                self.not_burger_basket7,
                not_burger_option_7,
                Not_burger_menu,
            )
        )
        self.not_burger_option8.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_8.product,
                self.not_burger_description8,
                self.not_burger_basket8,
                not_burger_option_8,
                Not_burger_menu,
            )
        )
        self.not_burger_option9.configure(
            command=lambda: add_to_basket_window(
                self,
                not_burger_option_9.product,
                self.not_burger_description9,
                self.not_burger_basket9,
                not_burger_option_9,
                Not_burger_menu,
            )
        )
        ...
        # --->back button
        self.not_burger_back.configure(
            command=lambda: controller.show_frame(Menu_navigation_frame)
        )
        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
        ...
        # --->top row frame
        self.not_burger_menu_top_frame.grid(
...
class Dessert_menu(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.dessert_var1 = tk.StringVar(
            self, f"{dessert_option_1.product} \n\n £{dessert_option_1.price}"
        )
        self.dessert_var2 = tk.StringVar(
            self, f"{dessert_option_2.product} \n\n £{dessert_option_2.price}"
        )
        self.dessert_var3 = tk.StringVar(
            self, f"{dessert_option_3.product} \n\n £{dessert_option_3.price}"
        )
        self.dessert_var4 = tk.StringVar(
            self, f"{dessert_option_4.product} \n\n £{dessert_option_4.price}"
        )
        self.dessert_var5 = tk.StringVar(
            self, f"{dessert_option_5.product} \n\n £{dessert_option_5.price}"
        )
        self.dessert_var6 = tk.StringVar(
            self, f"{dessert_option_6.product} \n\n £{dessert_option_6.price}"
        )
        self.dessert_var7 = tk.StringVar(
            self, f"{dessert_option_7.product} \n\n £{dessert_option_7.price}"
        )
        self.dessert_var8 = tk.StringVar(
            self, f"{dessert_option_8.product} \n\n £{dessert_option_8.price}"
        )
        self.dessert_var9 = tk.StringVar(
            self, f"{dessert_option_9.product} \n\n £{dessert_option_9.price}"
        )
        ...
        # --->logo
        self.dessert_menu_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.dessert_menu_logo_widget = ctk.CTkLabel(
            self.dessert_menu_top_frame, image=self.dessert_menu_logo, text=""
        )

        # --->title
        self.dessert_menu_title = ctk.CTkLabel(
            self.dessert_menu_top_frame,
            textvariable=self.dessert_title_var,
            font=("TkHeadingFont", 30),
        )
        ...
        self.dessert_menu_basket_button = ctk.CTkButton(
            self.dessert_menu_basket_frame,
            textvariable=self.dessert_basket_button_var,
            font=("TkHeadingFont", 10),
            cursor="hand2",
            hover_color="#BC006C",
            height=10,
        ...
        # --->item buttons
        self.dessert_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_1.product,
                self.dessert_description1,
                self.dessert_basket1,
                dessert_option_1,
                Dessert_menu,
            )
        )
        self.dessert_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_2.product,
                self.dessert_description2,
                self.dessert_basket2,
                dessert_option_2,
                Dessert_menu,
            )
        )
        self.dessert_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_3.product,
                self.dessert_description3,
                self.dessert_basket3,
                dessert_option_3,
                Dessert_menu,
            )
        )
        self.dessert_option4.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_4.product,
                self.dessert_description4,
                self.dessert_basket4,
                dessert_option_4,
                Dessert_menu,
            )
        )
        self.dessert_option5.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_5.product,
                self.dessert_description5,
                self.dessert_basket5,
                dessert_option_5,
                Dessert_menu,
            )
        )
        self.dessert_option6.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_6.product,
                self.dessert_description6,
                self.dessert_basket6,
                dessert_option_6,
                Dessert_menu,
            )
        )
        self.dessert_option7.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_7.product,
                self.dessert_description7,
                self.dessert_basket7,
                dessert_option_7,
                Dessert_menu,
            )
        )
        self.dessert_option8.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_8.product,
                self.dessert_description8,
                self.dessert_basket8,
                dessert_option_8,
                Dessert_menu,
            )
        )
        self.dessert_option9.configure(
            command=lambda: add_to_basket_window(
                self,
                dessert_option_9.product,
                self.dessert_description9,
                self.dessert_basket9,
                dessert_option_9,
                Dessert_menu,
            )
        )
        ...
        # --->frame for basket button and basket total
        self.dessert_menu_basket_frame.grid(
            row=0, column=2, padx=(0, 5), pady=6, sticky="nsew"
        )
...
class Drinks_menu(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.drinks_var1 = tk.StringVar(
            self, f"{drinks_option_1.product} \n\n £{drinks_option_1.price}"
        )
        self.drinks_var2 = tk.StringVar(
            self, f"{drinks_option_2.product} \n\n £{drinks_option_2.price}"
        )
        self.drinks_var3 = tk.StringVar(
            self, f"{drinks_option_3.product} \n\n £{drinks_option_3.price}"
        )
        self.drinks_var4 = tk.StringVar(
            self, f"{drinks_option_4.product} \n\n £{drinks_option_4.price}"
        )
        self.drinks_var5 = tk.StringVar(
            self, f"{drinks_option_5.product} \n\n £{drinks_option_5.price}"
        )
        self.drinks_var6 = tk.StringVar(
            self, f"{drinks_option_6.product} \n\n £{drinks_option_6.price}"
        )
        self.drinks_var7 = tk.StringVar(
            self, f"{drinks_option_7.product} \n\n £{drinks_option_7.price}"
        )
        self.drinks_var8 = tk.StringVar(
            self, f"{drinks_option_8.product} \n\n £{drinks_option_8.price}"
        )
        self.drinks_var9 = tk.StringVar(
            self, f"{drinks_option_9.product} \n\n £{drinks_option_9.price}"
        )
        ...
        # --->logo
        self.drinks_menu_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.drinks_menu_logo_widget = ctk.CTkLabel(
            self.drinks_menu_top_frame, image=self.drinks_menu_logo, text=""
        )

        # --->title
        self.drinks_menu_title = ctk.CTkLabel(
            self.drinks_menu_top_frame,
            textvariable=self.drinks_title_var,
            font=("TkHeadingFont", 30),
        )
        ...
        self.drinks_menu_basket_button = ctk.CTkButton(
            self.drinks_menu_basket_frame,
            textvariable=self.drinks_basket_button_var,
            font=("TkHeadingFont", 10),
            cursor="hand2",
            hover_color="#BC006C",
            height=10,
        ...
        # --->item buttons
        self.drinks_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_1.product,
                self.drinks_description1,
                self.drinks_basket1,
                drinks_option_1,
                Drinks_menu,
            )
        )
        self.drinks_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_2.product,
                self.drinks_description2,
                self.drinks_basket2,
                drinks_option_2,
                Drinks_menu,
            )
        )
        self.drinks_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_3.product,
                self.drinks_description3,
                self.drinks_basket3,
                drinks_option_3,
                Drinks_menu,
            )
        )
        self.drinks_option4.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_4.product,
                self.drinks_description4,
                self.drinks_basket4,
                drinks_option_4,
                Drinks_menu,
            )
        )
        self.drinks_option5.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_5.product,
                self.drinks_description5,
                self.drinks_basket5,
                drinks_option_5,
                Drinks_menu,
            )
        )
        self.drinks_option6.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_6.product,
                self.drinks_description6,
                self.drinks_basket6,
                drinks_option_6,
                Drinks_menu,
            )
        )
        self.drinks_option7.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_7.product,
                self.drinks_description7,
                self.drinks_basket7,
                drinks_option_7,
                Drinks_menu,
            )
        )
        self.drinks_option8.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_8.product,
                self.drinks_description8,
                self.drinks_basket8,
                drinks_option_8,
                Drinks_menu,
            )
        )
        self.drinks_option9.configure(
            command=lambda: add_to_basket_window(
                self,
                drinks_option_9.product,
                self.drinks_description9,
                self.drinks_basket9,
                drinks_option_9,
                Drinks_menu,
            )
        ...
        # --->frame for basket button and basket total
        self.drinks_menu_basket_frame.grid(
            row=0, column=2, padx=(0, 5), pady=6, sticky="nsew"
        )
...
class Milkshake_menu(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        self.milkshake_var1 = tk.StringVar(
            self, f"{milkshake_option_1.product} \n\n £{milkshake_option_1.price}"
        )
        self.milkshake_var2 = tk.StringVar(
            self, f"{milkshake_option_2.product} \n\n £{milkshake_option_2.price}"
        )
        self.milkshake_var3 = tk.StringVar(
            self, f"{milkshake_option_3.product} \n\n £{milkshake_option_3.price}"
        )
        self.milkshake_var4 = tk.StringVar(
            self, f"{milkshake_option_4.product} \n\n £{milkshake_option_4.price}"
        )
        self.milkshake_var5 = tk.StringVar(
            self, f"{milkshake_option_5.product} \n\n £{milkshake_option_5.price}"
        )
        self.milkshake_var6 = tk.StringVar(
            self, f"{milkshake_option_6.product} \n\n £{milkshake_option_6.price}"
        )
        self.milkshake_var7 = tk.StringVar(
            self, f"{milkshake_option_7.product} \n\n £{milkshake_option_7.price}"
        )
        self.milkshake_var8 = tk.StringVar(
            self, f"{milkshake_option_8.product} \n\n £{milkshake_option_8.price}"
        )
        self.milkshake_var9 = tk.StringVar(
            self, f"{milkshake_option_9.product} \n\n £{milkshake_option_9.price}"
        )
        ...
        # --->logo
        self.milkshake_menu_logo = ctk.CTkImage(
            dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
        )
        self.milkshake_menu_logo_widget = ctk.CTkLabel(
            self.milkshake_menu_top_frame, image=self.milkshake_menu_logo, text=""
        )

        # --->title
        self.milkshake_menu_title = ctk.CTkLabel(
            self.milkshake_menu_top_frame,
            textvariable=self.milkshake_title_var,
            font=("TkHeadingFont", 30),
        )
        ...
        self.milkshake_menu_basket_button = ctk.CTkButton(
            self.milkshake_menu_basket_frame,
            textvariable=self.milkshake_basket_button_var,
            font=("TkHeadingFont", 10),
            cursor="hand2",
            hover_color="#BC006C",
            height=10,
        ...
        # --->option buttons
        self.milkshake_option1.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_1.product,
                self.milkshake_description1,
                self.milkshake_basket1,
                milkshake_option_1,
                Milkshake_menu,
            )
        )
        self.milkshake_option2.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_2.product,
                self.milkshake_description2,
                self.milkshake_basket2,
                milkshake_option_2,
                Milkshake_menu,
            )
        )
        self.milkshake_option3.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_3.product,
                self.milkshake_description3,
                self.milkshake_basket3,
                milkshake_option_3,
                Milkshake_menu,
            )
        )
        self.milkshake_option4.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_4.product,
                self.milkshake_description4,
                self.milkshake_basket4,
                milkshake_option_4,
                Milkshake_menu,
            )
        )
        self.milkshake_option5.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_5.product,
                self.milkshake_description5,
                self.milkshake_basket5,
                milkshake_option_5,
                Milkshake_menu,
            )
        )
        self.milkshake_option6.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_6.product,
                self.milkshake_description6,
                self.milkshake_basket6,
                milkshake_option_6,
                Milkshake_menu,
            )
        )
        self.milkshake_option7.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_7.product,
                self.milkshake_description7,
                self.milkshake_basket7,
                milkshake_option_7,
                Milkshake_menu,
            )
        )
        self.milkshake_option8.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_8.product,
                self.milkshake_description8,
                self.milkshake_basket8,
                milkshake_option_8,
                Milkshake_menu,
            )
        )
        self.milkshake_option9.configure(
            command=lambda: add_to_basket_window(
                self,
                milkshake_option_9.product,
                self.milkshake_description9,
                self.milkshake_basket9,
                milkshake_option_9,
                Milkshake_menu,
            )
        )
        ...
        # --->frame for basket button and basket total
        self.milkshake_menu_basket_frame.grid(
            row=0, column=2, padx=(0, 5), pady=6, sticky="nsew"
        )
...
def add_to_basket_window(master, product, description, price, instance, menu):
    ...
    down_arrow = ctk.CTkImage(
        Image.open(r"Project\images\down_arrow.png"), size=(20, 20)
    )

    product_title = ctk.CTkLabel(
        add_to_basket_frame, text=product, font=("TkHeadingFont", 30)
    )
    ...
    quantity_increase_button = ctk.CTkButton(
        add_to_basket_frame, image=up_arrow, text="", width=20, height=20
    )
    quantity_count = ctk.CTkLabel(add_to_basket_frame, textvariable=quantity_var)
    quantity_decrease_button = ctk.CTkButton(
        add_to_basket_frame, image=down_arrow, text="", width=20, height=20
    )
    ...
    # --->place widgets on grid<---#
    product_title.grid(
        column=0, row=0, columnspan=5, padx=10, pady=(20, 10), sticky="nsew"
    )
    product_description.grid(
        column=0, row=1, columnspan=5, rowspan=3, padx=20, pady=10, sticky="nsew"
    )
...
def basket_window():
    ...
    def load_window():
        ...
        basket_window_frame = ctk.CTkFrame(
            basket_window, fg_color="#161219", border_color="#BC006C", border_width=5
        )
        basket_window_frame.grid(column=0, row=0, sticky="nsew")

        basket_window_title = ctk.CTkLabel(
            basket_window_frame, text="Your Basket", font=("TkHeadingFont", 30)
        )
        basket_window_title.grid(
            row=0, column=0, columnspan=5, padx=5, pady=(10, 0), sticky="nsew"
        )
        ...
        order_summary_frame.grid(
            row=1, column=0, columnspan=5, rowspan=5, padx=15, pady=10, sticky="nsew"
        )
        ...
        delete_button = ctk.CTkButton(
            basket_window_frame, text="Delete Item", command=delete_item
        )
        ...
        checkout_button = ctk.CTkButton(
            ...
            command=lambda: checkout_window(basket_window),
        )
        checkout_button.grid(column=1, row=9, padx=30, pady=(5, 20), sticky="nsew")

        back_button = ctk.CTkButton(
            basket_window_frame, text="Back", command=basket_window.destroy
        )
...
def checkout_window(basket_window):
    ...
    checkout_window_frame = ctk.CTkFrame(
        checkout_window, fg_color="#161219", border_color="#BC006C", border_width=5
    )
    ...
    # --->top row frame
    checkout_menu_top_frame = ctk.CTkFrame(
        checkout_window_frame,
        fg_color="#161219",
        border_color="#BC006C",
        border_width=5,
    )
    ...
    # --->logo
    checkout_menu_logo = ctk.CTkImage(
        dark_image=Image.open(r"Project\images\Logo15.png"), size=(50, 50)
    )
    checkout_menu_logo_widget = ctk.CTkLabel(
        checkout_menu_top_frame, image=checkout_menu_logo, fg_color="#161219", text=""
    )

    # --->title
    checkout_menu_title = ctk.CTkLabel(
        checkout_menu_top_frame,
        text="Checkout",
        text_color="white",
        fg_color="#161219",
        font=("TkHeadingFont", 30),
    ...
    # --->frame for basket button and basket total
    checkout_menu_basket_frame = ctk.CTkFrame(
        checkout_menu_top_frame, fg_color="#161219"
    )

    # --->basket navigation
    checkout_menu_basket_button = ctk.CTkButton(
        checkout_menu_basket_frame,
        text="Basket",
        text_color="white",
        fg_color="#161219",
        font=("TkHeadingFont", 10),
        cursor="hand2",
        command=basket_window,
        hover_color="#BC006C",
        height=10,
        width=30,
        border_width=0,
    ...
    checkout_menu_top_frame.grid(
        row=0, column=0, pady=(0, 7.5), columnspan=3, sticky="nsew"
    )
    ...
    first_name_entry.grid(
        column=1, row=1, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    surname_label.grid(column=0, row=2, padx=(5, 0), pady=7.5, sticky="nsew")
    surname_entry.grid(
        column=1, row=2, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    house_num_label.grid(column=0, row=3, padx=(5, 0), pady=7.5, sticky="nsew")
    house_num_entry.grid(
        column=1, row=3, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    street_name_label.grid(column=0, row=4, padx=(5, 0), pady=7.5, sticky="nsew")
    street_name_entry.grid(
        column=1, row=4, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    town_city_label.grid(column=0, row=5, padx=(5, 0), pady=7.5, sticky="nsew")
    town_city_entry.grid(
        column=1, row=5, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    county_label.grid(column=0, row=6, padx=(5, 0), pady=7.5, sticky="nsew")
    county_entry.grid(
        column=1, row=6, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    post_code_label.grid(column=0, row=7, padx=(5, 0), pady=7.5, sticky="nsew")
    post_code_entry.grid(
        column=1, row=7, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    phone_number_label.grid(column=0, row=8, padx=(5, 0), pady=7.5, sticky="nsew")
    phone_number_entry.grid(
        column=1, row=8, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )

    email_address_label.grid(column=0, row=9, padx=(5, 0), pady=7.5, sticky="nsew")
    email_address_entry.grid(
        column=1, row=9, columnspan=2, padx=(0, 15), pady=7.5, sticky="nsew"
    )
...
def place_order(
    ...
    try:
        ...
        if house_num_var.get().isnumeric() == False:
            raise ValueError
        elif re.search(r"^[a-z]{1,2}[a-z0-9]{1,2}\s[0-9]{1}[a-z]{2}$", post_code_var.get(), re.IGNORECASE) == None:
            raise ValueError
        elif re.search(r"^[+0]{1}[74]{1}[0-9]{9,14}$", phone_number_var.get()) == None:
            raise ValueError
        else:
            generate_receipt(email_address_var.get(), first_name_var.get())
            confirmation_window(
                first_name_var.get(), email_address_var.get(), checkout_window
            )
            clear_basket()
    except ValueError:
        ...
        def error_window():
            ...
            error_window_frame = ctk.CTkFrame(
                error_window, fg_color="#161219", border_color="#BC006C", border_width=5
            )
            error_window_title_label = ctk.CTkLabel(error_window_frame, text="Error")
            error_window_label = ctk.CTkLabel(
                error_window_frame,
                text="Please ensure the details you've entered are correct before proceeding,\n\nThank you.",
            )

            error_window_frame.grid(column=0, row=0, sticky="nsew")
            error_window_title_label.grid(column=0, row=0, sticky="nsew")
            error_window_label.grid(column=0, row=1, sticky="nsew")

            error_window.columnconfigure(0, weight=1)
            error_window.rowconfigure(0, weight=1)
            error_window_frame.columnconfigure(0, weight=1)
            error_window_frame.rowconfigure(0, weight=1)
            error_window_frame.rowconfigure(1, weight=1)
...
def confirmation_window(customer_name, customer_email, checkout_window):
    ...
    confirmation_window_frame = ctk.CTkFrame(
        confirmation_window, fg_color="#161219", border_color="#BC006C", border_width=5
    )

    confirmation_title_label = ctk.CTkLabel(
        confirmation_window_frame, text="Order Confirmed"
    )

    confirmation_label = ctk.CTkLabel(
        confirmation_window_frame,
